...
fn main() {
    ...
    while start.elapsed() < time_limit  {
        ...
        while target_positions.len() != 0 {
            ...
            println!("hold: {holding_1} {holding_2}");
            let start_pos ;
            match nearest_point(root_x, root_y, &mut takoyaki) {
                Some(value) => start_pos = value,
                None => {
                    takoyaki_none = true;
                    start_pos = (root_x, root_y);
                },
            }
            ...
            if takoyaki_none {
                if holding_1 {
                    (tx, ty) = nearest_goal_in_grid1(sx, sy, ttx, tty, n).unwrap();
                } else {
                    (tx, ty) = nearest_goal_in_grid2(sx, sy, ttx, tty, n).unwrap();
                }
            } else {
                (tx, ty) = nearest_goal_in_grid(sx, sy, ttx, tty, n).unwrap();
            }
            ...
            while takoyaki_none != true && (root_x as isize - sx as isize).abs() + (root_y as isize - sy as isize).abs() != 0 {
            ...
            while takoyaki_none != true && dir_current != start_direction {
                let mut cmd = vec!['.'; v_prime*2];
                let diff = (start_direction + 4 - dir_current) % 4;
                if diff == 1 {
                    cmd[1] = 'R';
        ...
        while let Some(next_cmd) = action_queue.pop_front() {
            ...
            for i in 0..v_prime*2 {
                if current_cmd[7] == 'P' && current_cmd[8] == 'P' && current_cmd[9] == 'P' {
                    can_merge = false;
                    break;
...
fn nearest_goal_in_grid1(root_x: usize, root_y: usize, goal_x: usize, goal_y: usize, n: usize) -> Option<(usize, usize)> {
    let mut goals = HashSet::new();

    // (2, 1), (-1, 2), (1, -2), (-2, -1) から離れた位置に移動させる
    let offsets = [(2, 1), (-1, 2), (1, -2), (-2, -1)];

    for &(dx, dy) in &offsets {
        let new_x = goal_x as isize + dx;
        let new_y = goal_y as isize + dy;

        // グリッドの範囲内かを確認
        if new_x >= 0 && new_x < n as isize && new_y >= 0 && new_y < n as isize {
            goals.insert((new_x as usize, new_y as usize));
        }
    }
...
fn nearest_goal_in_grid2(root_x: usize, root_y: usize, goal_x: usize, goal_y: usize, n: usize) -> Option<(usize, usize)> {
    let mut goals = HashSet::new();

    let offsets = [(2, -1), (-1, -2), (-2, 1), (1, 2)];
...
fn pick_or_drop_takoyaki(
    ...
    let arm_pos_1u = ((root_x as isize + arm_positions[0].0).abs() as usize, (root_y as isize + arm_positions[0].1).abs() as usize);
    let arm_pos_2u = ((root_x as isize + arm_positions[2].0).abs() as usize, (root_y as isize + arm_positions[2].1).abs() as usize);


    if !*holding_1 {
        if takoyaki.contains(&arm_pos_1u) && (sx, sy) != arm_pos_1u {
    ...
    if !*holding_2 {
        if takoyaki.contains(&arm_pos_2u) && (sx, sy) != arm_pos_2u {
