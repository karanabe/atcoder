...
#[inline]
fn inside(x: isize, y: isize, n: usize) -> bool {
    0 <= x && x < n as isize && 0 <= y && y < n as isize
}
        ...
        for y in 0..n {
            ...
            for (k, &(dx, dy)) in DIRS.iter().enumerate() {
                ...
                loop {
                    ...
                    if !inside(nx, ny, n) || board[nx as usize][ny as usize] {
                        break;
                    }
                    cx = nx;
                    cy = ny;
...
fn choose_next(
    ...
    for (x, row) in nxt0.iter().enumerate().take(n) {
        for (y, &value) in row.iter().enumerate().take(n) {
            if board[x][y] {
                continue;
            }
            cand.push((x, y, value));
    ...
    for &(x, y, q) in &cand {
        let mut b1 = board.to_vec();
        let mut d1 = dest.clone();
        let mut p1 = nxt0.clone();
        b1[x][y] = true;
        p1[x][y] = 0.0;
        let factor = 1.0 - q;
        if factor > 0.0 {
    ...
    for node in &beam {
        ...
        for (x, row) in nxt.iter().enumerate().take(n) {
            for (y, &value) in row.iter().enumerate().take(n) {
                if node.board[x][y] {
                    continue;
                }
                cand2.push((x, y, value));
            }
        }
...
fn greedy_step(
    ...
    for (x, row) in nxt.iter().enumerate().take(n) {
        for (y, &value) in row.iter().enumerate().take(n) {
            ...
            let key = (value, x, y);
            if key.partial_cmp(&best).unwrap() == Ordering::Less {
                best = key;
            }
        }
    }
...
#[inline]
fn choose_two(r: &mut StdRng, len: usize) -> (usize, usize) {
...
#[inline]
fn choose_range(r: &mut StdRng, len: usize) -> (usize, usize) {
    let l = r.gen_range(0..len - 1);
    let r2 = r.gen_range(l + 1..len);
    (l, r2)
}
...
fn main() {
    ...
    for (x, row) in board.iter().enumerate().take(n) {
        for (y, &cell) in row.iter().enumerate().take(n) {
            if !cell {
                empty.push((x, y));
            }
        }
    ...
    while start.elapsed().as_secs_f64() < SA_TIME && rem_seq.len() > 5 {
        ...
        let len = rem_seq.len();
        let (l, r) = if action < 80 {
            choose_two(&mut rng, len)
        } else {
            choose_range(&mut rng, len)
        };
    ...
    while seq.len() < n * n - m {
        let (bx, by, _minp) = greedy_step(&board, &dest, &prob, n);
        seq.push((bx, by));
        board[bx][by] = true;
        update_dest_one(&mut dest, &board, bx, by, n);
        prob[bx][by] = 0.0;
