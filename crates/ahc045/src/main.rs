...
fn try_local_move(
    ...
    match op_type {
        0 => {
            if n_gr < 2 { return (old_total, vec![], record); }
            ...
            let new_total = recalc_cost_for_groups(groups, groups_cost, dist_model, &changed, old_total);
            (new_total, changed, record)
        },
        1 => {

            if n_gr < 3 { return (old_total, vec![], record); }
...
fn simulated_annealing(
    ...
    while Instant::now() < end_t {
        ...
        let (new_cost, changed, record) = try_local_move(groups, &mut groups_cost, dist_model, total_cost, op_type, &mut rng);
        ...
        if delta < 0.0 {
            ...
            if rng.gen::<f64>() >= p {
                rollback_local_move(groups, &mut groups_cost, dist_model, &changed, total_cost, &record);
            } else {
                total_cost = new_cost;
            }
        }
    ...
    eprintln!("SA finished. iter={}, best_cost={:.2}", iter_count, best_score);
...
fn main() {
    ...
    for start in (0..n).step_by(step) {
        if used_q >= q_max { break; }
        if start + l > n { break; }
        let sub = boxes[start..start+l].iter().map(|&(id,_,_)| id).collect_vec();
        subsets.push(sub);
        used_q+=1;
    }
...
fn bfs_clustering(n: usize, coords: &[(f64,f64)], threshold: f64) -> Vec<Vec<usize>> {
    ...
    idxs.sort_by(|&a,&b| {
        coords[a].0.partial_cmp(&coords[b].0).unwrap_or(Ordering::Equal)
    });
    let check_range = 120;
    for i in 0..n {
        let ci = idxs[i];
        for j in i+1..(i+check_range).min(n) {
            ...
            if dx*dx > threshold*threshold { break; }
...
fn build_not_adopted_edges(sub: &[usize], adopted: &[(usize,usize,usize)], large_rank: usize)
